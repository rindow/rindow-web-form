<?php
namespace Rindow\Web\Form\View;
...
class FormRenderer
{
    ...
    public function __construct(array $themes=null,$translator=null,$textDomain=null,$hidePassword=null)
    {
        $this->themes = $themes;
    ...
    public function setHidePassword($hidePassword)
    {
        $this->hidePassword = $hidePassword;
    }
    ...
    public function raw(ElementInterface $element,$attributes=array())
    {
        ...
        if($attributes===array()) {
            ...
            if(isset($theme['field'])) {
                if(isset($theme['field'][$element->type]))
                    $type = $element->type;
                else
                    $type = 'default';
                if($element->errors!==null && count($element->errors)!=0)
                    ...
                    $status = 'success';
                if(isset($theme['field'][$type][$status]['field']))
                    $fieldDivAttr  = $theme['field'][$type][$status]['field'];
                if(isset($theme['field'][$type][$status]['label']))
                    $labelDivAttr  = $theme['field'][$type][$status]['label'];
                if(isset($theme['field'][$type][$status]['widget']))
                    $widgetDivAttr = $theme['field'][$type][$status]['widget'];
            }
    ...
    public function widgetRadio(ElementSelectionInterface $element,array $attributes=array())
    {
        ...
        unset($attributes['id']);
        $optionLabelDivClass = false;
        if(isset($attributes['optionLabelDivClass'])) {
            $optionLabelDivClass = $attributes['optionLabelDivClass'];
        }
        unset($attributes['optionLabelDivClass']);
        foreach($element as $el) {
            ...
            if($el->label) {
                if($optionLabelDivClass)
                    $html .= '<div class="'.$optionLabelDivClass.'">';
        ...
        foreach($attributes as $name => $value) {
            if($value!==null) {
                if(isset(self::$needToTranslate[$name][$type])) {
                    if($type=='password' && $name=='value' && $this->hidePassword)
                        $html .= ' '.$name.'=""';
                    else
    ...
    public function addElement(ElementCollectionInterface $elementCollection,$type,$name,$value=null,$label=null,$attributes=null)
    {
    ...
    public function setView($renderer)
    {
